	                    ...
	                    <tr>
	                        ...
	                        <td>
				 				<a href="#" onclick="viewPermission('${user.name}')" class="btn btn-success btn-xs" data-toggle="modal" data-target="#editModal">
				 					...
				 					<a href="${pageContext.request.contextPath}/deleteEmployee?id=${user.id}" class="btn btn-danger btn-xs"><span class="glyphicon glyphicon-remove"></span> 删除角色</a>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<table class="table table-bordered table-striped" width="800px">
						<tr>
							<td>帐号</td>
							<td><input class="form-control" name="name" id="name" placeholder="名称"></td>
						</tr>					
						<tr>
							<td>初始密码</td>
							<td><input class="form-control" type="password" name="password" id="password"  placeholder="名称"></td>
						</tr>
						<tr>
						    <td>电子邮箱</td>
						    <td><input class="form-control" name="email" id="email" placeholder="链接">
						    </td>
						</tr>
						<tr>
						    <td>级别</td>
						    <td>
						    	<select class="form-control" name="role" onchange="getNextManager(this.value)" id="role">
  ...
  <div class="modal-dialog" >
	...
	$(function(){
        ...
        if (message=='该用户已被注册') {
			...
			alert("注册成功");
		}else if(message=='该邮箱已被注册'){
			alert("该邮箱已被注册");
		}else if(message=='账号不能为空'){
			alert("账号不能为空");
		}else if(message=='密码不能为空'){
			alert("密码不能为空");
		}else if(message=='邮箱不能为空'){
			alert("邮箱不能为空");
		}else if(message=='上级主管不能为空'){
			alert("注册失败,上级主管不能为空");
